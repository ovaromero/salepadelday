...
const TeamSetup: FC<TeamSetupProps> = ({ onStart }) => {
  ...
  return (
    <div className="max-w-2xl mx-auto px-2 sm:p-4">
      <h1 className="text-2xl sm:text-3xl font-bold text-center mb-6 sm:mb-8 text-indigo-600 uppercase tracking-widest">SalePadelDay</h1>
      <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-6">
        <div className="flex justify-center gap-2 sm:space-x-4 mb-4 sm:mb-6">
          <button
            type="button"
        ...
        {teams.slice(0, teamCount).map((team, tIdx) => (
          <div key={team.id} className="bg-white p-4 sm:p-6 rounded-2xl shadow-md border-l-4 sm:border-l-8 border-indigo-500 hover:shadow-lg transition-shadow">
            <input
              type="text"
              value={team.name}
              onChange={(e) => handleTeamNameChange(tIdx, e.target.value)}
              placeholder="Nombre del Equipo"
              className="w-full text-lg sm:text-xl font-bold mb-3 sm:mb-4 border-b-2 border-transparent focus:border-indigo-300 outline-none p-1 transition-colors"
              required
            />
              ...
              {team.players.map((player, pIdx) => (
                <div key={pIdx}>
                  ...
                  <input
                    ...
                    placeholder="Nombre"
                    className="w-full bg-gray-50 rounded-lg p-2 sm:p-3 outline-none focus:ring-2 focus:ring-indigo-300 transition-all border border-gray-200 text-sm sm:text-base"
                  />
                </div>
              ))}
            </div>
        ...
        <button
          type="submit"
          className="w-full bg-green-500 text-white py-3 sm:py-4 rounded-2xl text-lg sm:text-xl font-black uppercase tracking-tighter hover:bg-green-600 active:scale-95 transition-all shadow-xl"
        >
          ¡Comenzar Torneo!
        </button>
      </form>
