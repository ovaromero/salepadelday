...
const PastJourneys: FC<PastJourneysProps> = ({ journeys, onDelete, onBack }) => {
  ...
  return (
    <div className="max-w-2xl mx-auto p-4 flex flex-col min-h-screen">
      <div className="flex items-center mb-8">
        ...
        <h1 className="text-3xl font-black text-gray-800 tracking-tighter uppercase italic">Historial</h1>
      </div>

      <div className="space-y-6">
        {journeys.map(journey => (
          <div key={journey.id} className="bg-white rounded-3xl p-6 shadow-md border border-gray-100 group relative">
            <button 
              onClick={() => handleDelete(journey.id)}
              className="absolute top-4 right-4 text-red-300 hover:text-red-500 transition-colors p-2"
            ...
            <div className="mb-4">
              ...
              <h2 className="text-xl font-black text-gray-800 uppercase tracking-tighter">Jornada de {journey.teams.length} Equipos</h2>
            </div>

            <div className="grid grid-cols-2 gap-4 mb-6">
              <div className="bg-gray-50 rounded-2xl p-4 text-center">
                <p className="text-[10px] font-black uppercase text-gray-400 mb-1">Partidos</p>
                <p className="text-2xl font-black text-gray-800">{journey.history.length}</p>
              </div>
              <div className="bg-gray-50 rounded-2xl p-4 text-center">
                ...
                <p className="text-sm font-black text-indigo-600 uppercase">
                  {/* Simple logic: team with most wins */}
                  {(() => {
            ...
            <details className="group/details">
              ...
              <div className="mt-4 space-y-2 max-h-60 overflow-y-auto pr-2">
                {journey.history.map((match, mIdx) => (
                  <div key={mIdx} className="flex items-center justify-between bg-gray-50/50 p-3 rounded-xl border border-gray-100">
