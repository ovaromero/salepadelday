  ...
  return (
    <div className="max-w-2xl mx-auto p-4 flex flex-col min-h-screen">
      <div className="flex items-center mb-8">
        <button onClick={onBack} className="mr-4 p-2 text-indigo-600 hover:bg-indigo-50 rounded-full transition-colors">
          <svg xmlns="http://www.w3.org/2000/svg" className="h-8 w-8" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M15 19l-7-7 7-7" />
          </svg>
        </button>
        <h1 className="text-3xl font-black text-gray-800 tracking-tighter uppercase italic">Estadísticas</h1>
      </div>
          ...
          <div className="bg-white rounded-3xl shadow-lg overflow-hidden border border-gray-100">
            <table className="w-full text-left">
              <thead className="bg-gray-50 border-b border-gray-100">
                <tr>
                  <th className="px-6 py-4 text-xs font-bold text-gray-400 uppercase">Equipo</th>
                  <th className="px-6 py-4 text-xs font-bold text-gray-400 uppercase text-center">G</th>
                  <th className="px-6 py-4 text-xs font-bold text-gray-400 uppercase text-center">P</th>
                </tr>
              </thead>
                  ...
                  <tr key={team.id} className={idx === 0 ? 'bg-indigo-50/30' : ''}>
                    <td className="px-6 py-4">
                      <div className="flex items-center">
                        ...
                        <div>
                          <p className="font-black text-gray-800 uppercase tracking-tight">{team.name}</p>
                          <p className="text-xs text-gray-500 font-bold uppercase">{team.players[0].name} / {team.players[1].name}</p>
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-4 text-center font-black text-indigo-600 text-xl">{team.wins}</td>
                    <td className="px-6 py-4 text-center font-bold text-gray-400 text-lg">{team.losses}</td>
                  </tr>
                ))}
              </tbody>
            </table>
            ...
            {completedMatches.slice().reverse().map((match, idx) => (
              <div key={idx} className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm flex items-center justify-between">
                <div className="flex-1 flex flex-col items-center">
                   <span className={`font-black uppercase tracking-tighter text-sm ${match.result?.winnerTeamId === match.team1.id ? 'text-indigo-600' : 'text-gray-400'}`}>
                     {match.team1.name}
                   </span>
                </div>
                
                <div className="px-4 text-center">
                   <div className="bg-indigo-600 text-white px-3 py-1 rounded-lg font-black text-sm mb-1 italic">
                ...
                <div className="flex-1 flex flex-col items-center">
                   <span className={`font-black uppercase tracking-tighter text-sm ${match.result?.winnerTeamId === match.team2.id ? 'text-indigo-600' : 'text-gray-400'}`}>
                     {match.team2.name}
                   </span>
                </div>
              </div>
